---
name: summarize
description: Summarize a research source, paper, report, repository README, local artifact, or PDF using the RLM pattern — source stored on disk, never injected raw into context. Use when the user asks to summarize a paper, URL, PDF, repo, or long document.
argument-hint: <source> [--window-size <chars>] [--overlap <chars>] [--tier1-threshold <chars>] [--tier2-threshold <chars>]
---

# Summarize (RLM)

Summarize the research source the user names.
...
- `--window-size <chars>` or `SUMMARIZE_WINDOW_CHARS` (default: `6000`)
- `--overlap <chars>` or `SUMMARIZE_OVERLAP_CHARS` (default: `500`)
- `--tier1-threshold <chars>` or `SUMMARIZE_TIER1_THRESHOLD` (default: `8000`)
- `--tier2-threshold <chars>` or `SUMMARIZE_TIER2_THRESHOLD` (default: `60000`)
...
- **Remote URL**: fetch to disk with `curl -sL -o outputs/.notes/<slug>-raw.txt <url>` via Bash. Do NOT use WebFetch — its return value enters context directly, bypassing the RLM external-variable principle.
...
Spawn one `Agent` call per chunk, all in a single message so they run in parallel (cap concurrency sensibly, e.g. 4-6 at a time for very large documents):

`Agent(subagent_type="researcher", prompt="Read ONLY outputs/.notes/<slug>-chunk-NNN.txt. Extract: (1) key claims, (2) methodology or technical approach, (3) cited evidence. Do NOT use web search or fetch external URLs — this is single-source summarization. If a claim appears to start or end mid-sentence at the file boundary, mark it BOUNDARY PARTIAL. Write to outputs/.notes/<slug>-summary-chunk-NNN.md.")`
...
## Field Context
[Where the source positions itself, which prior work or research line it claims to extend, and what remains source-inferred rather than externally checked. Do not invent author/lab background; use the literature-review skill for a corpus-level view.]
